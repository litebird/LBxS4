...
class FiltCoadd:
    ...
    def __find_internal_params__(self):
        if self.option == 'LB':
            ...
            self.beamb = np.reshape(self.lblib.beamb,(1,self.lmax+1))
    ...
    def __QU__(self,Alm,which):
        if which == 'E':
            Q,U = hp.alm2map([Alm*0,Alm,Alm*0],self.nside)[1:]
        elif which == 'B':
            Q, U = hp.alm2map([Alm*0,Alm*0,Alm],self.nside)[1:]
        else:
    ...
    def QU(self,idx,which):
        if self.option == 'LB':
            if which == 'E':
                Alm = self.lblib.NILC_Elm(idx)
            elif which == 'B':
                Alm = self.lblib.NILC_Blm(idx)
            else:
                raise ValueError("Unknown which")
            QU = self.__QU__(Alm, which)
            del Alm
            return QU
        elif self.option == 'S4':
            Elm = self.s4lib.NILC_Elm(idx)
            QU = self.__QU__(Elm,'E')
            del Elm
            return QU
        elif self.option == 'LBxS4':
            lbElm = self.lblib.NILC_Elm(idx)
            lbQU = self.__QU__(lbElm,'E')
            del lbElm
            s4Elm = self.s4lib.NILC_Elm(idx)
            s4QU = self.__QU__(s4Elm,'E')
            del s4Elm
    ...
    def __invNL__(self,lib,idx,which,shaped=True):
        if which == 'E':
            ne = lib.NILC_ncl(idx)/self.Tcmb**2
        elif which == 'B':
            ne = lib.NILC_nclb(idx)/self.Tcmb**2
        else:
            raise valueError("Unknown which")
    ...
    def invNL(self,idx,which='E'):
        if self.option == 'LB':
            return self.__invNL__(self.lblib,idx,which)
        elif self.option == 'S4':
            return self.__invNL__(self.s4lib,idx,which='E')
        elif self.option == 'LBxS4':
            lbs4_NL = np.zeros((2,2,self.lmax+1))
            lbs4_NL[0,0,:] = self.__invNL__(self.lblib,idx,which='E',shaped=False)
            lbs4_NL[0,1,:] = self.__invNL__(self.s4lib,idx,which='E',shaped=False)
            return lbs4_NL
        else:
            raise ValueError("Unknown option")
    ...
    def filtBmode(self,idx,wiener=False,eps=1e-4,status=None):
        fname = os.path.join(self.libdir,f"cinv_B_{idx:04d}.pkl")
        if not os.path.isfile(fname):
            QU = self.QU(idx,which='B')
            NL = self.invNL(idx,which='B')
            if (self.option == 'LB') or (self.option == 'S4'):
                channels = 1
            else:
                channels = 2
            E,B = cs.cninv.cnfilter_freq(2,channels,self.nside,self.lmax,self.cl_len[1:3,:self.lmax+1],
                                    self.beamb, self.ninv,QU,chn=1,itns=[1000],filter="",
                                    eps=[eps],ro=10,inl=NL,stat=status)
            del E
            pl.dump(B,open(fname,'wb'))
            del (QU,NL)
        else:
            B = pl.load(open(fname,'rb'))
        
        if wiener:
            clbb = self.cl_len[2,:self.lmax+1]
            B = cs.utils.almxfl(self.lmax,self.lmax,B,clbb)
            return B
        else:
            return B
    ...
    def plot_W_B(self,idx,lmin=2,lmax=1000,ymin=1e-18,ymax=1e-14):
        E = self.filtBmode(idx,wiener=True)
        we = cs.utils.alm2cl(self.lmax,E)
        plt.figure(figsize=(5,5))
        plt.loglog(we/self.fsky)
        plt.loglog(self.cl_len[2,:])
        #plt.xlim(lmin,lmax)
        #plt.ylim(ymin,ymax)
        plt.legend(['Wiener B','Input B'], fontsize=15)
        plt.xlabel('$\ell$',fontsize=20)
        plt.ylabel('$C_\ell$',fontsize=20)
